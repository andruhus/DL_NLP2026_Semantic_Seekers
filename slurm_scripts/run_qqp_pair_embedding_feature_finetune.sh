...
mkdir -p experiments/qqp/pair_features_finetune_predictions
mkdir -p experiments/qqp/pair_features_finetune_checkpoints
...
EXP_PRED_DIR="experiments/qqp/pair_features_finetune_predictions"
EXP_CKPT_DIR="experiments/qqp/pair_features_finetune_checkpoints"

cp predictions/bert/quora-paraphrase-dev-output.csv \
  "$EXP_PRED_DIR/quora_pair_features_finetune_job${SLURM_JOB_ID}_dev.csv"

cp predictions/bert/quora-paraphrase-test-output.csv \
  "$EXP_PRED_DIR/quora_pair_features_finetune_job${SLURM_JOB_ID}_test.csv"

if [ -f models/finetune-3-1e-05-qqp.pt ]; then
  cp models/finetune-3-1e-05-qqp.pt \
    "$EXP_CKPT_DIR/qqp_pair_features_finetune_job${SLURM_JOB_ID}.pt"
fi

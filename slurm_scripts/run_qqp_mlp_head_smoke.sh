...
mkdir -p experiments/qqp_mlp_head/smoke_test_predictions
mkdir -p experiments/qqp_mlp_head/smoke_test_checkpoints
...
EXP_PRED_DIR="experiments/qqp_mlp_head/smoke_test_predictions"
EXP_CKPT_DIR="experiments/qqp_mlp_head/smoke_test_checkpoints"

cp predictions/bert/quora-paraphrase-dev-output.csv \
  "$EXP_PRED_DIR/quora_mlp_head_smoke_job${SLURM_JOB_ID}_dev.csv"

cp predictions/bert/quora-paraphrase-test-output.csv \
  "$EXP_PRED_DIR/quora_mlp_head_smoke_job${SLURM_JOB_ID}_test.csv"

if [ -f models/finetune-1-1e-05-qqp.pt ]; then
  cp models/finetune-1-1e-05-qqp.pt \
    "$EXP_CKPT_DIR/qqp_mlp_head_smoke_job${SLURM_JOB_ID}.pt"
fi

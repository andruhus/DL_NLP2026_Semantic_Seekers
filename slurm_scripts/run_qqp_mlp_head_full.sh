...
mkdir -p experiments/qqp_mlp_head/full_run_predictions
mkdir -p experiments/qqp_mlp_head/full_run_checkpoints
...
rm -f models/finetune-3-1e-05-qqp.pt
...
EXP_PRED_DIR="experiments/qqp_mlp_head/full_run_predictions"
EXP_CKPT_DIR="experiments/qqp_mlp_head/full_run_checkpoints"

cp predictions/bert/quora-paraphrase-dev-output.csv \
  "$EXP_PRED_DIR/quora_mlp_head_full_job${SLURM_JOB_ID}_dev.csv"

cp predictions/bert/quora-paraphrase-test-output.csv \
  "$EXP_PRED_DIR/quora_mlp_head_full_job${SLURM_JOB_ID}_test.csv"

if [ -f models/finetune-3-1e-05-qqp.pt ]; then
  cp models/finetune-3-1e-05-qqp.pt \
    "$EXP_CKPT_DIR/qqp_mlp_head_full_job${SLURM_JOB_ID}.pt"
fi
